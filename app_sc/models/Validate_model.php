...
class Validate_model extends CI_Model
{
    ...
    function failMessage($data, $params = null){
        $this->validation['type'] = 'danger';
            $this->validation['msg'] = $this->validation['msg'] . $data ."</br>";
            $this->validation['bool'] = false;
            if(isset($params)){
                $this->validation['params'] = $params;
            }
            
    }

    function confirmMessage($data){
        $this->validation['type'] = $this->validation['type'] == 'success' ||  $this->validation['type'] == 'confirmPass'? 'warning' : $this->validation['type'];
        $this->validation['cnfrmMsg'] = $this->validation['cnfrmMsg'] . $data ."</br>";
        $this->validation['bool'] = false;
    }

    function confirmPass($data){
        $this->validation['type'] = $this->validation['type'] == 'success' ? 'confirmPass' : $this->validation['type'];
        $this->validation['cnfrmPs'] = $this->validation['cnfrmPs'] . $data ."</br>";
        $this->validation['bool'] = false;
    }
      
    function validate($form, $data){
        $this->validation = array('bool' => true, 'msg' => "", 'cnfrmMsg' => "", 'cnfrmPs' => "", 'type' => 'success', 'auth' => 'pass');
        switch ($form) {
            case 'applyRefunds' :
        ...
        if(empty($this->validation['msg'])){
            $this->validation['msg'] = $this->validation['cnfrmMsg'];
            if(empty($this->validation['msg'])){
                $this->validation['msg'] = $this->validation['cnfrmPs'];
            }
        }
...
// function checkDeleteLines($deletes){
//     $cleared = '';
//     $deposited = '';

//     $this->db->select('t.id, t.rec_id, t.clr, t.deposit_id');
//     $this->db->from('transactions t');
//     $this->db->where_in('t.id', $deletes);
//     $this->db->where('((t.rec_id IS NOT NULL AND t.clr = 1) OR (t.deposit_id IS NOT NULL AND t.deposit_id != 0)');
//     $q = $this->db->get();
//         if($q->num_rows() > 0) {
...
// }
    ...
    function account($data){
        ...
        $aid = $data['id'];
        $required = array('Account Name' => $account, 'Account Type' => $data['account']['account_types_id']);
        $this->requiredFields('account', $required );

        $this->accountName($account, $aid);
        $this->gl($account['accno'], $aid);
    ...
    function checkPropertyEntityDate($data, $closing){
        ...
        foreach($transactions as $singleTransactions){
            if(array_key_exists('credit', $singleTransactions) ? ($singleTransactions['credit'] > 0 or $singleTransactions['credit'] < 0) : (array_key_exists('debit', $singleTransactions) ? ($singleTransactions['debit'] > 0 or $singleTransactions['debit'] < 0) : ($singleTransactions['amount'] > 0 or $singleTransactions['amount'] < 0 ))  or $data['header']['vcheck']){
    ...
    function checkDate($date1, $date2, $closing = null, $password = null){
        ...
        if(strtotime($date1) < strtotime($date2) && $date1 !=null && $date2 !=null){
        //query for password
        // if ($q->num_rows() > 0) {
        //     $data = $q->row();
        ...
        // if($data->password == $password) return;
                if($closing == null){
                    ...
                    //$this->failMessage("Transaction date is before closing date for this entity");
                    return;
                }
    ...
    public function billConfirm($data)
    {
        ...
        $this->db->where('TRIM(th.transaction_ref)', trim($data['header']['transaction_ref']));
        ...
        if($data['trans_id'])$this->db->where('th.id !=', $data['trans_id']);
        //$this->db->where('t.profile_id', $data['headerTransaction']['credit']);
        $q = $this->db->get();
        if ($q->num_rows() > 0) {
            $this->confirmMessage('A bill for this property with this reference and vendor already exists');
        }
    }
    ...
    function bills($data){
        ...
        $this->checkPropertyEntityDate($checkPropertyEntity, 1);
        if($data['deletes']) $this->checkDeleteLines($data['deletes']);
    ...
    function checks($data){
        ...
        $this->transactionClosed($transaction['trans_id'], $headerTransaction, 1);
        ...
        //$propertyTransaction = array();
        foreach($headerTransaction as &$transaction){
            $this->transactionClosed($transaction['id'], $transaction );
            $transactions[$transaction['id']] = $transaction;
            $transaction['amount'] = (double)$transaction['credit'];
        }
        ...
        //bankTrans validation
        function bankTrans($data){
            $header = $data['header'];
            $transactions = $data['transactions'];
            $headerTransaction = $data['headerTransaction'];
            //$special = $data['special'];
    ...
    function vchecks($data){
        ...
        foreach($transactions as &$transaction){
            ...
            $this->PropertyTotal($propertyTransaction);
            //$this->checkTotal($transactions, $headerTransaction['credit']);
            //$this->requiredFields( 'vchecks', $header, $headerTransaction);
    ...
    function deposits($data){
        if($data['deletes']){
            $this->checkDeleteLines($data['deletes']);
            return;
        }
        ...
        // if($data['mode'] === 'edit'){
        //     $this->checkDeleteLines($data['trans_id'], $data['property']);
        // }
    ...
    public function checkDeleteLines($deletes){
    ...
    function receivePayments($data){
        ...
        if($rec != NULL && $rec != 0){
            if (round($amount,2) != $curAmnt){
                return $this->failMessage('Hey! you can\'t change the amount on a cleared payment');
            } else if ($data['transactions'][0]['account_id'] != $data['curAcct']) {
                return $this->failMessage('Sorry you can\'t change the deposit to account on a cleared payment');
            }
    ...
    function transfer_bal($data){
        $prp1 = $data['transfer1']['prop_id'];
        $prp2 = $data['transfer2']['prop_id'];

        if($prp1 != $prp2){
            $this->failMessage('Sorry, you can\'t transfer a tenant\'s balance to a different property');
        }

        if($data['transfer1']['lease_id'] == $data['transfer2']['lease_id']  && $data['profile1'] == $data['profile2']){
            $this->failMessage('You are transfering to the same account!');
        }
        $this->sufficientFunds($from, $data['amount']);
        $this->requiredFields('transfer_bal', $data);
    }
    ...
    function utilities($data){
     ...
     function accountName($account, $aid){
        $q = $this->db->select('name, id')->from('accounts')->where('account_types_id', $account['account_types_id'])->where('name', $account['name'])->where('id !=', $aid)->where('parent_id', $account['parent_id'])->get();
        if ($q->num_rows() > 0) {
                    $account['id'] = $aid;
                    $params = Array('account_a' => $account, 'account_b' => $q->row(0));
                    $this->failMessage("Account name is already used.", $params);
                    //$this->account_a($account);
                    //$this->account_b($q->result(0));
                }
     }

     function gl($accno, $aid){
        $this->db->select('accno');
        $this->db->from('accounts');
        $this->db->where('TRIM(accno)', trim($accno));
        if($aid) $this->db->where('id !=', $aid);
        $q = $this->db->get();
                      
        if ($q->num_rows() > 0) {
            $this->failMessage("GL# is already used.");
        }
     }
    ...
    function checkProfileLease($transaction){
        ...
        if($profile_id == "" || $profile_id == "0"){return;}
    ...
    function requiredFields( $type, $header, $topData = null){
        ...
        unset($topData['request_approval_from']);
    ...
    //can't change account or amount if transaction has a rec id that closed = 1
    function transactionClosed($tid, $transaction, $checkheader = null){
         $this->load->model('transactions_model');
         $oldTransaction = new stdClass();
         if ($checkheader){
            $oldTransaction = $this->transactions_model->getCheckHeaderTransaction($transaction['trans_id']);
         } else {
           ...
           if($oldTransaction->rec_id != Null && $oldTransaction->clr == 1 ){
                    if($transaction['credit']){
                        if(round($oldTransaction->credit,2) != round($transaction['credit'],2)) {
                            $this->failMessage("This transaction is cleared already, the amount can't be changed.");
                        }
                    }
                    ...
                    if(trim($oldTransaction->account_id) != trim($transaction['account_id'])){
                        $this->failMessage("This transaction is cleared already, the account can't be changed.");
                    }
           } elseif ($oldTransaction->rec_id != Null){
                if(trim($oldTransaction->account_id) != trim($transaction['account_id'])){
                    $this->failMessage("This transaction is marked for reconcilliation, you need to remove it from the reconcilliation in order to change the account.");
                }
           }
    }
