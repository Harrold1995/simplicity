...
class Ion_auth_model extends CI_Model
{
    ...
    public function login($identity, $password, $remember = false)
    {
        ...
        $query = $this->db->select('users.'.$this->identity_column . ', users.email as email, users.id as id, password, users.active as active, last_login, profiles.first_name as first_name, profiles.last_name as last_name, image, profiles.id as profileId, users.email_password as email_password, colors')
    ...
    public function users($groups = null)
    {
        ...
        if (isset($this->_ion_select) && !empty($this->_ion_select)) {
            ...
            $this->db->select(array(
                ...
                'p.first_name as first_name1',
                'p.last_name as last_name1'
            ));
            $this->db->join('profiles p', 'p.id = users.profile_id', 'left');
        }
    ...
    public function get_users_groups_ids($id = false)
    {

        // if no id was passed use the current users id
        $id || $id = $this->session->userdata('user_id');
        
        $groups = $this->get_users_groups($id);
    ...
    public function set_session($user)
    {
        ...
        $session_data = array(
            ...
            'colors' => $user->colors ? $user->colors: '#e884c7;rgba(243,124,228,.7);#fb5fe7;#E884C7;#f56aa0;#fb5ee9',
            'company_name' => $this->site->settings->company_name,
            //'company_address' => $user->profileId,
    ...
    public function login_remembered_user()
    {
        ...
        // get the user
        $this->trigger_events('extra_where');
        $query = $this->db->select($this->identity_column . ', id, email, last_login ,colors')
