...
$GLOBALS['FPDM_FILTERS']=array(); //holds all supported filters
$GLOBALS['FPDM_REGEXPS']= array(
	"/Type"=>"/\/Type\s+\/(\w+)$/",
	"/Subtype" =>"/^\/Subtype\s+\/(\w+)$/"
);
...
//					$this->Error("set_field_value failed as invalid valuetype $type for object $object_id");
		...
		**/
		...
		//---------------------
			return implode($sep,$GLOBALS['FPDM_FILTERS']);
		}
		...
		**/
		...
		//-----------------------------------------------------------
				$value=preg_match($GLOBALS['FPDM_REGEXPS']["$name"],$line,$match);
		...
		//-----------------------------------------------------------
				if(array_key_exists($name,$GLOBALS['FPDM_REGEXPS'])) {
