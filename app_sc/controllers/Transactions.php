...
//include_once 'checks.php';
    ...
    public function checkTransactions()//run everyday maybe crontab? set on linux server or task scheduler on windows if using windows server?
    {
        ...
        $combined = array();
        $this->db->select('mt.id, mt.transaction_type, f.interval_unit, f.number, mt.end_date, mt.next_trans_date, mt.data, l.bill_collectively, l.id as lease_id');
        ...
        $this->db->where('mt.next_trans_date = "'.$entryDate.'" OR mt.next_trans_date < "'.$entryDate.'"');
        ...
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as &$row) {
                $success = $this->applyTransaction($row);
                if($this->site->settings->email_autocharge_notices == 1 && $success){
                    if($row->bill_collectively == 1){
                        $rowDetails = json_decode($row->data);
                        $combined[$row->lease_id]['items'][$rowDetails->transactions->item_id]['amount'] += $rowDetails->transactions->credit;
                        $combined[$row->lease_id]['row'] = $success['row'];
                    } else {
                        $this->sendEmail($success['row'],'c');
                    }
            ...
            foreach ($combined as $lease) {
                $messageBody = 'A new charge has been posted to your account: <table style ="width: 90%; text-align: center">';
                foreach ($lease['items'] as $key => $item) {
                    $itemName = $this->db->get_where('items', array('id' => $key))->row('sales_description');
                    $lease['row']['credit'] = $item['amount'];
                    $messageBody = $messageBody.'<tr><td>'.$itemName.'</td><td>$'.number_format($item['amount'], 2).'</td></tr>';
                
                }
                $data['message'] = $messageBody.'</table>';
                $this->load->model('tenants_model');
                $email = $this->tenants_model->getAllLeaseTenantsEmail($lease['row']['lease_id']);
                $data['tenant']->first_name = $this->tenants_model->getAllLeaseTenantsNames($lease['row']['lease_id']);
                ...
                $subject = 'A new charge has been posted to your account';
    ...
    Function addManagementChecks(){

        $fees = $this->input->post('row');
        $check_date =  date('Y-m-d', strtotime($this->input->post('check_date')));
        $as_of =  date('Y-m-d', strtotime($this->input->post('as_of_date')));
        $nextDate = date('Y-m-d', strtotime($as_of . ' +1 day'));
        
        $result = array();
        $toPrint = array();
        
        foreach ($fees as $fee){
           $_POST = json_decode('{"headerTransaction":{"account_id":"'.$fee['default_bank'].'","credit":"'.$fee['calcAmount'].'","profile_id":"'.$fee['vendor'].'","class_id":"", "property_id":"'.$fee['property_id'].'"},"header":{"transaction_ref":"","transaction_date":"'.$check_date.'","memo":"'.$fee['memo'].'","to_print":"1"},"transactions":[{"account_id":"'.$fee['account_id'].'","property_id":"'.$fee['property_id'].'","unit_id":"'.$fee['unit_id'].'","description":"","debit":"'.$fee['calcAmount'].'","class_id":"","profile_id":"'.$fee['vendor'].'"}],"radioButton":"normal", "bills":{"terms":"30 days","due_date":"2022\/04\/28","request_approval_from":"20270"}}', true);
           $_POST['header']['transaction_date'] = $check_date;
           $_POST['saveAndPrint'] = '1';
           
           $success = $this->addCheck(0,1);
            if ($success) {
                $this->db->query('UPDATE management_fees SET start_date ="'.$nextDate.'" WHERE id =' . $fee['id']);
                $result[$fee['id']] = 'success';
                array_push($toPrint,$success[0]);
   ...
   public function checkManagementFees()//run everyday maybe crontab? set on linux server or task scheduler on windows if using windows server?
    {
        ...
        $this->db->select('f.interval_unit, f.number, management_fees.id, management_fees.item_id, p.default_bank, management_fees.frequency, management_fees.account_id, management_fees.percentage_fixed, management_fees.vendor, management_fees.amount, management_fees.property_id, p.name');
        ...
        if ($q->num_rows() > 0) {
            foreach (($q->result()) as &$row) {
                ...
                if($row->percentage_fixed == 1){
                    ...
                    $this->db->where('transaction_header.transaction_date >', $startDate);
                    $this->db->where('transaction_header.transaction_date <=', $endDate);
                    ...
                    $q2 = $this->db->get();
                        if ($q2->num_rows() > 0) {
                            foreach (($q2->result()) as &$mfee) {
                                echo $mfee->amount.'<br>';
                                if ($mfee->amount > 0 ){
                                    $row->memo = $row->amount.' percent of charged income totaling $'.$mfee->amount.' from '.$startDate.' to '.$endDate.'.';
                            ...
                            echo 'no income for'.$row->property_id;
                            //but need to update next date
                            continue;
                        }
                ...
                $_POST['header']['transaction_date'] = $today;
                 ...
                 $success = $this->site->settings->management_check == 1 ? $this->addCheck(): $this->addBill();
    ...
    public function applyTransaction($transaction)
    {
        ...
        switch ($transaction->transaction_type) {
            ...
            case 5 ://customer payments
                ...
                break;
            case 6 ://charge
                $success = $this->newCharge(null, true);
                break;
            case 7://bill payment
    ...
    function addBill($memorized = null)
    {
        ...
        foreach($transactions as &$transaction){
            if ($transaction['debit'] <> 0 or $transaction['credit'] <> 0) {
                $headerTransactions[$transaction['property_id']]['credit'] = $headerTransactions[$transaction['property_id']]['credit'] + $transaction['debit']-$transaction['credit'];
                $headerTransactions[$transaction['property_id']]['property_id'] = $transaction['property_id'];
                $headerTransactions[$transaction['property_id']]['account_id'] = $this->ap;
                $headerTransactions[$transaction['property_id']]['profile_id'] = $headerTransaction['profile_id'];
                $headerTransactions[$transaction['property_id']]['class_id'] = $headerTransaction['class_id'];
            } 
        }
        ...
        $validate['bool'] && $validate['auth'] =='pass') {
            $transaction_id_b = $this->bills_model->addbill($type, $header, $headerTransactions, $transactions, $special);
            if ($transaction_id_b) {
                if ($saveAndPay == 1) {
    ...
    function editBill($tid = 0)
    {
        ...
        $headerTransaction = $this->removeHeaderComma($this->input->post('headerTransaction'));
        $transactions = $this->removeDetailCommas($this->input->post('transactions'));
        $header_ids = explode(",", $headerTransaction['id']);
        $headerTransactions = array();
        $headerTransactionAdd = array();
        unset($headerTransaction['credit']);
        foreach ($transactions as &$transaction) {
            if ($transaction['debit'] <> 0 or $transaction['credit'] <> 0) {

                //for validation
                $headerTransaction['credit'] += $transaction['debit'];
                $headerTransaction['credit'] -= $transaction['credit'];
                $headerTransaction['trans_id'] = $header['id'];


                if (array_key_exists($transaction['property_id'],$headerTransactions))
                {
                    $headerTransactions[$transaction['property_id']]['credit'] = $headerTransactions[$transaction['property_id']]['credit'] + $transaction['debit']-$transaction['credit'];
                }
                else
                {   
                    $trs = Array(
                        ...
                        'account_id' => $this->ap,
                        'profile_id' => $headerTransaction['profile_id'],
                        'rec_id' => $headerTransaction['rec_id'],
                        'clr' => $headerTransaction['clr'],
                        'trans_id' => $tid,
                        'class_id' => $headerTransaction['class_id']
                        
                    );
                    if (count($header_ids)>0){
                        $trs['credit'] = $headerTransactions[$transaction['property_id']]['amount'] + $transaction['debit'] - $transaction['credit'];
                        $trs['id'] = array_shift($header_ids); 
                        $headerTransactions[$transaction['property_id']] = $trs;                   
                    } else {
                        if (array_key_exists($transaction['property_id'],$headerTransactionAdd))
                            {
                                $headerTransactionAdd[$transaction['property_id']]['credit'] = $headerTransactionAdd[$transaction['property_id']]['credit'] + $transaction['debit']-$transaction['credit'];
                            } else {
                                $trs['credit'] = $transaction['debit'] - $transaction['credit'];
                                $headerTransactionAdd[$transaction['property_id']] = $trs; 
                            }
        ...
        if (/*$this->form_validation->run() &&*/
        $validate['bool'] && $validate['auth'] =='pass') {
            $transaction_id_b = $this->bills_model->editBill($type, $header, $headerTransactions, $transactions, $special, $tid, $deletes, $applied, $origAmt, $headerTransactionAdd);
            if ($transaction_id_b) {
                if ($saveAndPay == 1) {
                    $this->db->select('default_bank');
                    $this->db->from('properties');
    ...
    function addInvoice($memorized = null)
    {
        ...
        $header = $this->input->post('header');
        $headerTransaction = $this->removeHeaderComma($this->input->post('headerTransaction'));
        $headerTransaction['account_id'] = $this->ar;
        $transactions = $this->removeDetailCommas($this->input->post('transactions'));
        $special = $this->input->post('invoice');
        ...
        if($radioButton =='normal'){
            $headerTransaction['debit'] = $headerTransaction['credit'] + $headerTransaction['debit'];
            unset($headerTransaction['credit']);
        } else {
            $headerTransaction['credit'] = $headerTransaction['credit'] + $headerTransaction['debit'];
            unset($headerTransaction['debit']);
        }
        //$this->form_validation->set_rules($this->settings->accountFormValidation);
        foreach ($transactions as $key=>$trans){
            //$transactions[$key]['account_id'] = $this->invoice_model->getAccountId($trans['item_id']);
            $transactions[$key]['property_id'] = $headerTransaction['property_id'];
            if($radioButton =='normal'){
                $transactions[$key]['credit'] = $trans['credit'] + $trans['debit'];
                unset($transactions[$key]['debit']);
            } else {
                $transactions[$key]['debit'] = $trans['credit'] + $trans['debit'];
                unset($transactions[$key]['credit']);
            }
        }
        
        $data = array('header' => $header, 'headerTransaction' => $headerTransaction, 'transactions' => $transactions, 'special' => $special, 'confirm' => $confirm);
        $validate = $this->validate_model->validate("invoice", $data);
        if ($memorized) {
            return $validate;
        }
        ...
        if (/*$this->form_validation->run() &&*/
        $validate['bool'] && $validate['auth'] =='pass') {
            $transaction_id_b = $this->invoice_model->addInvoice($radioButton, $header, $headerTransaction, $transactions, $special);
            if ($transaction_id_b) {
                
                echo json_encode(array('type' => $validate['type'], 'auth' => $validate['auth'], 'message' => 'Invoice successfully added.'));
                return true;
            } else {
                echo json_encode(array('type' => 'danger', 'auth' => $validate['auth'], 'message' => 'Something went wrong'));
            }
    ...
    function editInvoice($tid = 0)
    {
        ...
        if (/*$this->form_validation->run() &&*/
        $validate['bool'] && $validate['auth'] =='pass') {
            $transaction_id_b = $this->invoice_model->editInvoice($radioButton, $header, $headerTransaction, $transactions, $special, $tid, $deletes, $applied, $origAmt);
            if ($transaction_id_b) {
               
                echo json_encode(array('type' => $validate['type'], 'auth' => $validate['auth'], 'message' => 'Invoice successfully edited.'));
            } else {
                echo json_encode(array('type' => 'danger', 'auth' => $validate['auth'], 'message' => 'Something went wrong'));
            }
        } else {
    ...
    function invoicegetModal()
    {
        ...
        switch ($this->input->post('mode')) {
            case 'add' :
                $params = json_decode($this->input->post('params'));
                $this->data['target'] = 'transactions/addInvoice';
                $this->data['title'] = 'Add Invoice';
                $this->data['classes'] = $this->invoice_model->getClasses();
                $this->data['jClasses'] = json_encode($this->data['classes']);
                $this->data['properties'] = $this->invoice_model->getProperties();
                $this->data['jProperties'] = json_encode($this->data['properties']);
                $this->data['accounts'] = $this->invoice_model->getAllAccounts();
                $this->data['subaccounts'] = $this->site->getNestedSelect($this->data['accounts']);
                $this->data['jAccounts'] = json_encode($this->data['accounts']);
                $this->data['jsubaccounts'] = json_encode($this->data['subaccounts']);
                $this->data['units'] = $this->invoice_model->getUnits();
                $this->data['subunits'] = $this->site->getNestedSelect($this->data['units']);
                $this->data['jsubunits'] = json_encode($this->data['subunits']);
                $this->data['jUnits'] = json_encode($this->data['units']);
                $this->data['names'] = $this->invoice_model->getNames();
                $this->data['jNames'] = json_encode($this->data['names']);
                $this->data['employees'] = $this->invoice_model->getAllEmployees();
                $this->data['jTransactions'] = json_encode($this->data['transactions']);
                //$this->data['jPropertyAccounts'] = json_encode($this->checks_model->getPropertyAccounts2());
                $this->data['paidStatusHtml'] = "";
                $this->data['headerTransaction']->profile_id = $params->profile;
                ...
                break;
            case 'edit' :
                $this->data['target'] = 'transactions/editInvoice/' . $this->input->post('id');
                ...
                $this->data['units'] = $this->invoice_model->getUnits();
                $this->data['subunits'] = $this->site->getNestedSelect($this->data['units']);
                $this->data['header'] = $this->invoice_model->getHeader($this->input->post('id'));
                $transactions = $this->invoice_model->getTransactions($this->input->post('id'));
                //$transactions =  $this->data['allTransactions'];
                $headerTransaction = array_shift($transactions);
                $this->data['transactions'] = $transactions;
                $this->data['headerTransaction'] = $headerTransaction;
                $this->data['paid'] = $this->invoice_model->getAmountPaid($this->input->post('id'));
                $this->data['bills'] = $this->invoice_model->getBill($this->input->post('id'));
                $this->data['names'] = $this->invoice_model->getNames();
                $this->data['jClasses'] = json_encode($this->data['classes']);
                $this->data['jProperties'] = json_encode($this->data['properties']);
                $this->data['jAccounts'] = json_encode($this->data['accounts']);
        ...
        $this->load->view('forms/invoice/invoice_form', $this->data);
    }


    ////charges

    function newCharge($validate = null, $memorized = null)
    {
        $errors = "";
        ...
        if (/*$this->form_validation->run() &&*/
            $validate['bool'] && $validate['auth'] =='pass' && $this->charges_model->addCharge($header, $transaction)) {
                ...
                if ($memorized) {
                    ...
                    $validate['row'] = $transaction;
                    return $validate;
                }
                 if($this->site->settings->email_autocharge_notices == 1){
                    $row = $transaction;
                    $row['date'] = $header['transaction_date'];
    ...
    function addBankTrans($memorized = null)
    {
        ...
        $radioButton = $this->input->post('radioButton');
        $headerTransaction = $this->removeHeaderComma($this->input->post('headerTransaction'));
        if($this->input->post('bankTransId')){
            $rec_id = $this->insertRec($headerTransaction['account_id']);
            $transAmount = $this->input->post('bankTransAmt');
        }
        $transactions = $this->removeDetailCommas($this->input->post('transactions'));
        $headerTransactions = array();
        foreach($transactions as &$transaction){
            if ($transaction['debit'] <> 0 or $transaction['credit'] <> 0) {
                if($radioButton =='credit'){
                    $headerTransactions[$transaction['property_id']]['credit'] = $headerTransactions[$transaction['property_id']]['credit'] + $transaction['debit']-$transaction['credit'];
                    $totalAmount += $transaction['debit'] - $transaction['credit'];
                } else {
                    $headerTransactions[$transaction['property_id']]['debit'] = $headerTransactions[$transaction['property_id']]['debit'] + $transaction['debit'];
                ...
                $headerTransactions[$transaction['property_id']]['property_id'] = $transaction['property_id'];
                $headerTransactions[$transaction['property_id']]['account_id'] = $headerTransaction['account_id'];
                $headerTransactions[$transaction['property_id']]['profile_id'] = $headerTransaction['profile_id'];
                $headerTransactions[$transaction['property_id']]['class_id'] = $headerTransaction['class_id'];

                if($this->input->post('bankTransId')){
                    $headerTransactions[$transaction['property_id']]['rec_id'] = $rec_id;
                    $headerTransactions[$transaction['property_id']]['clr'] = 1;                   
                }
                
            } 
        }

        $data = array('header' => $header, 'headerTransaction' => $headerTransactions, 'transactions' => $transactions);
        $validate = $this->validate_model->validate("bankTrans", $data);
        if ($memorized) {
            return $validate;
        }
        $amount2 = round((float)$this->input->post('bankTransAmt'),2);
        $roundtotal = round($totalAmount,2);

        if($this->input->post('bankTransId') ){
            $amount2 = round((float)$this->input->post('bankTransAmt'),2);
            $roundtotal = round($totalAmount,2);
           if($amount2 <> $roundtotal) {
                $accName = $this->db->get_where('accounts', array('id' => $this->input->post('bankTransAcct')))->row()->name;
                $total1 = isset($headerDebit) ? $this->input->post('headerDebit') : 0- $this->input->post('headerCredit');
                $this->db->delete('reconciliations', array('id' => $rec_id));
                echo json_encode(array('type' => 'danger', 'message' => 'Because you are entering this as a matched transaction, the amount for <b>'.$accName.'</b> must equal to $'.$amount2));
               return;
           }
        }

        if ($validate['bool'] && $validate['auth'] =='pass') {
            $checksInfo = $this->bankTrans_model->addBankTrans($header, $headerTransactions, $transactions);
            if ($checksInfo) {

                $array = ['type' => $validate['type'],'auth' => $validate['auth'], 'message' => "Bank Transaction Successfully added"];
                if($this->input->post('bankTransId')){
                    $this->db->update('plaid_trans', array('trans_match' => $rec_id), array('transaction_id' => $this->input->post('bankTransId')));
                    $array['recId'] = $rec_id;
                    $array['transId'] = $this->input->post('bankTransId');
                }
                echo json_encode($array);
                return true;
            } else {
                ...
                echo json_encode(array('type' => 'danger','auth' => $validate['auth'], 'message' => "Bank Transaction not added Something went wrong"));
                return false;
            }
        } else {
            $errors = $errors . validation_errors() . "</br>" . $validate['msg'];
    ...
    function editBankTrans($tid = 0)
    {
        ...
        foreach ($transactions as &$transaction) {
            if ($transaction['debit'] <> 0 or $transaction['credit'] <> 0) {
                ...
                if (array_key_exists($transaction['property_id'],$headerTransactions))
                {
                    if($radioButton =='credit'){
                        $headerTransactions[$transaction['property_id']]['credit'] = $headerTransactions[$transaction['property_id']]['credit'] + $transaction['debit']-$transaction['credit'];
                        $headerTransactions[$transaction['property_id']]['debit'] = 0;
                    } else {
                        $headerTransactions[$transaction['property_id']]['debit'] = $headerTransactions[$transaction['property_id']]['debit'] + $transaction['debit']-$transaction['credit'];
                        $headerTransactions[$transaction['property_id']]['credit'] = 0;
                        $transaction['credit'] = $transaction['debit'];
                        unset($transaction['debit']);
                    }
                ...
                {   
                    $trs = Array(
                        'property_id' => $transaction['property_id'],
                        'account_id' => $headerTransaction['account_id'],
                        'profile_id' => $headerTransaction['profile_id'],
                        'rec_id' => $headerTransaction['rec_id'],
                        'clr' => $headerTransaction['clr'],
                        'trans_id' => $header['id'],
                    ...
                    if (count($header_ids)>0){
                        if($radioButton =='credit'){
                            ...
                            $trs['debit'] =  $transaction['debit']-$transaction['credit'];
                            $trs['credit'] =  0;
                            $transaction['credit'] = $transaction['debit'];
                            unset($transaction['debit']);
                        }
                        ...
                        if (array_key_exists($transaction['property_id'],$headerTransactionAdd))
                            {   
                                if($radioButton =='credit'){
                                    $headerTransactionAdd[$transaction['property_id']]['credit'] = $headerTransactionAdd[$transaction['property_id']]['credit'] + $transaction['debit']-$transaction['credit'];
                                    $headerTransactionAdd[$transaction['property_id']]['debit'] = 0;
                                } else {
                                    $headerTransactionAdd[$transaction['property_id']]['debit'] = $headerTransactionAdd[$transaction['property_id']]['debit'] + $transaction['debit']-$transaction['credit'];
                                    $headerTransactionAdd[$transaction['property_id']]['credit'] = 0;
                                    $transaction['credit'] = $transaction['debit'];
                                    unset($transaction['debit']);
                                }
        ...
        $deletes = $this->input->post('delete') ?: array();

        //$m = $this->checks_model->msg;
        $data = array('header' => $header, 'headerTransaction' => $headerTransaction, 'headerTransactionsAdd' => $headerTransactionAdd, 'transactions' => $transactions, 'deletes' => $deletes);
        $validate = $this->validate_model->validate("checks", $data);
        //adding header deletes after validation to delete property line
        if(count($header_ids)>0){
            foreach($header_ids as $id){array_unshift($deletes,$id);}
         }
        ...
        if ($validate['bool'] && $validate['auth'] =='pass') {
            $checksInfo = $this->bankTrans_model->editBankTrans($header, $headerTransactions,$headerTransactionAdd, $transactions, $tid, $deletes);
            if ($checksInfo) {
    ...
    /// Reconciliation
    function insertRec($account_id)
    {
        $reconciliation['closed'] = 1;
        $reconciliation['beginning_bal'] = 1;
        ...
        $reconciliation['reconciled_by'] = $this->session->userdata('user_id');
    ...
    function addCheck($memorized = null, $returnPrint = null)
    {
        $this->load->model('checks_model');
        ...
        //     echo json_encode(array('type' => 'success', 'message' => 'Check successfully added.'));
        if ($validate['bool'] && $validate['auth'] =='pass') {
            $checksInfo = $this->checks_model->addCheck($header, $headerTransactions, $transactions, $print);
            if ($checksInfo) {
    ...
    function editCheck($tid = 0)
    {
        ...
        //     echo json_encode(array('type' => 'success', 'message' => 'Check successfully edited.'));
        if ($validate['bool'] && $validate['auth'] =='pass') {
            $checksInfo = $this->checks_model->editCheck($header, $headerTransactions,$headerTransactionAdd, $transactions, $tid, $print, $deletes);
            if ($checksInfo) {
                $array = ['type' => $validate['type'],'auth' => $validate['auth'], 'message' => "Check successfully edited"];
                if ($print === '1'){
    ...
    function voidCheck($tid = 0)
    {
        ...
        $headerTransactions = array();
        $headerTransaction['trans_id'] = $header['id'];
        $originalAmt = $headerTransaction['credit'];
        $headerTransaction['debit'] = '0.00';
        $headerTransaction['credit'] = '0.00';

        foreach($header_ids as &$header_id){
            $headerTransactions[$header_id]['debit'] = '0.00';
            $headerTransactions[$header_id]['credit'] = '0.00';
            $headerTransactions[$header_id]['id'] = $header_id;
        }

        $header['memo'] = 'VOID: Original Amt: $'.$originalAmt.' ' .$header['memo'];
        ...
        if ($validate['bool'] && $validate['auth'] =='pass') {
            if($delete == NULL){
                ...
                $checksInfo = $this->checks_model->editCheck($header, $headerTransactions, null, $transactions, $tid, $print, $deletes);
                if ($checksInfo) {
...
//nsf
    ...
    function nsfgetModal()
    {
        ...
        switch ($this->input->post('mode')) {
            case 'add':
                ...
                $this->data['lease'] = $params->{'profile[lease_id]'};
    ...
    function recIdHtml($recId){
        $rec = $this->db->get_where('reconciliations', array('id' => $recId))->row();
        if($rec->type != 'auto'){
          $clrd = $rec->closed ? 'clrdrecinfo' : 'unclrdrecinfo';
          return  ' <span data-id="50" class ="'.$clrd.' reportLink" rtype="report" defaults="'.$recId.'"> Cleared on Rec#'.$recId.'</span>';
        } else if($beginning_bal == 1){

        } else {
            return  ' <span data-id="50" class ="'.$clrd.' bankTransLink" data-rec="'.$recId.'"> Matched with '.$beginning_bal.' bank transactions</span>';
        }
    }
        ...
        public function bankTransgetModal()
        {
            ...
            switch ($this->input->post('mode')) {
                case 'add' :
                    ...
                    break;
                    case 'edit' :
                        $this->data['target'] = 'transactions/editBankTrans/' . $this->input->post('id');
                        $this->data['title'] = 'Edit Bank Trans';
                        $this->data['edit'] = 'edit';
                        $this->data['classes'] = $this->checks_model->getClasses();
                        $this->data['banks'] = $this->checks_model->getAllBanks();
                        $this->data['header'] = $this->checks_model->getHeader($this->input->post('id'));
    ...
    function depositPayments($process = false)
    {
        ...
        $rec_id = null;
        $total = $this->input->post('bankTransAmt');
        ...
        foreach($deposits as $key => $deposit){
            ...
            if($this->input->post('bankTransId') && $this->input->post('bankTransAcct') == $account_id){
                $deposit_to['rec_id'] = $rec_id;
                $deposit_to['clr'] = 1;
            ...
            if($process){
                ...
                if($this->input->post('bankTransId') && $this->input->post('bankTransAcct') == $account_id){
                   ...
                   if($amount2 <> $roundtotal) {
                       $accName = $this->db->get_where('accounts', array('id' => $this->input->post('bankTransAcct')))->row()->name;
                       echo json_encode(array('type' => 'danger', 'message' => 'Because you are entering this as a matched transaction, the the total deposit must equal to $'.$this->input->post('bankTransAmt')));
                       $this->db->delete('reconciliations', array('id' => $rec_id));
                       return;
                   }
                }
                if($this->deposits_model->depositPayments($header, $balance_undeposited_funds, $deposit_to, $undeposited, $otherDeposits)) {
                    ...
                    // delete rec id here if this matches the account id
                    if($this->input->post('bankTransId') && $this->input->post('bankTransAcct') == $account_id){
                        $this->db->delete('reconciliations', array('id' => $rec_id));
                    }
        ...
        foreach($properties as $property_id){
            if(trim($property_id) != ""){
                ...
                if($process){
                    //$i = 1;
                    if($this->input->post('bankTransAmt') && $this->input->post('bankTransAmt') <> $sepOtherDepositsAmount){
                        $accName = $this->db->get_where('accounts', array('id' => $this->input->post('bankTransAcct')))->row()->name;
                        echo json_encode(array('type' => 'danger', 'message' => 'Because you are entering this as a matched transaction, the total deposit must equal to $'.$this->input->post('bankTransAmt')));
                        $this->db->delete('reconciliations', array('id' => $rec_id));
                        return;
                    }
        ...
        if($process){
            if(count($fail) < 1){
                $response = array('type' => 'success','auth' => $validate['auth'], 'message' => $successes);

                if($this->input->post('bankTransId')){
                    $response['recId'] = $rec_id;
                    $response['transId'] = $this->input->post('bankTransId');
                }
        ...
        if($error === false){
            ...
            $err = implode(" ",$array); 
            echo json_encode(array('type' => 'danger','auth' => $validate['auth'], 'message' =>' ' . $err));
        }
    }
    ...
    //         if (($validate ? $validate['bool'] : 1) && $this->deposits_ model->depositPayments($header, $balance_undeposited_funds, $deposit_to, $undeposited, $otherDeposits)) {
    //             array_push($array, array('type' => 'success', 'message' => "Payments successfully deposited", 'msgInfo' => $property_id));
    //             //echo json_encode(array('type' => 'success', 'message' => 'Payments successfully deposited.'));
    //         } else {
    //             $errors = str_replace('</br>', ' ', $validate['msg']);
    ...
    {   
        ...
        if($properties[0] == "" && is_null($ufProperties)){
            echo json_encode(array('type' => 'danger', 'message' => 'You can not record an empty transaction.'));
            return;
        }
    ...
    function addJournalEntry($memorized = null)
    {
        ...
        if($this->input->post('bankTransId')){
            $rec_id = $this->insertRec($headerTransaction['account_id']);
            $headerDebit = $this->input->post('headerDebit');
            $total = isset($headerDebit) ? $this->input->post('headerDebit') : 0- $this->input->post('headerCredit');
            foreach($transactions as $key=>$transaction){
                if($this->input->post('bankTransAcct') == $transaction['account_id']){
                    $transactions[$key]['rec_id'] = $rec_id;
                    $transactions[$key]['clr'] = 1;
                    $total += $transaction['credit'] - $transaction['debit'];
                } else {
                    $transactions[$key]['rec_id'] = null;
                    $transactions[$key]['clr'] = null; 
                }
            }
            ...
            if ($total <> 0){
                ...
                $total1 = isset($headerDebit) ? $this->input->post('headerDebit') : 0- $this->input->post('headerCredit');
                $this->db->delete('reconciliations', array('id' => $rec_id));
                echo json_encode(array('type' => 'danger', 'message' => 'Because you are entering this as a matched transaction, the amount for <b>'.$accName.'</b> must equal to $'.$total1));
                return false;
            }
        }
            ...
            if ($response){
                if ($memorized) {
                   return $validate;
                    ...
                    $response1 = array('type' => $validate['type'],'auth' => $validate['auth'], 'message' => 'Transaction  successfully added.', 'id' => $response);
                    if($this->input->post('bankTransId')){
                        $response1['recId'] = $rec_id;
                        $response1['transId'] = $this->input->post('bankTransId');
                    }
...
//     }
    ...
    function addBillPayment(){
            ...
            $account_id = $data1['bank'];
            $property_id = $data1['property'];
            $profile_id = $data1['vendor'];
            $bills = $data1['applied_payments'];
            $header = $data1['header'];
            $checks = [];
            $array = [];
            $print = $this->input->post('saveAndPrint');
            ...
            if(count($bills) >1){
                //need to change this to split transactions in 2 or create a multiple property transaction
                echo json_decode(array('type' => 'danger', 'message' => "you can only add a bill payment for a single property"));
            }

            $transaction = reset($bills);

            foreach ($transaction as $key => $trans){
                if ($trans['applied'] == 0){unset($transaction[$key]);}
            }


            $pmt_account = ['account_id' => $account_id, 'profile_id' => $profile_id, 'property_id' => $property_id, 'credit' => $data1['amount']];
            $accounts_payable = ['account_id' => $this->ap, 'profile_id' => $profile_id, 'property_id' => $property_id, 'debit' => $data1['amount']];
            if($this->input->post('bankTransId')){
                if($this->input->post('bankTransAmt') <> $data1['amount']){
                    $accName = $this->db->get_where('accounts', array('id' => $account_id))->row()->name;
                    echo json_encode(array('type' => 'danger', 'message' => 'Because you are entering this as a matched transaction, the amount for <b>'.$accName.'</b> must equal to $'.$data1['amount']));
                    return false;
                }

                $rec_id = $this->insertRec($headerTransaction['account_id']);
                $headerDebit = $this->input->post('headerDebit');
                $pmt_account['rec_id'] = $rec_id;
                $pmt_account['clr'] = 1;
            }

            $data = ['header' => $header, 'transactions' => [0 => $pmt_account], 'transaction' => $transaction, 'mode' => 'add', 'print' => $data1['printEditBill']];
            $validate = $this->validate_model->validate("payBills", $data);

            if ($validate['bool'] && $validate['auth'] =='pass' ) {
                $checksInfo = $this->payBills_model->applyPayments($header, $transaction, $pmt_account, $accounts_payable, $print);
                if ($checksInfo) {
                    $response1 = array('type' => $validate['type'],'auth' => $validate['auth'], 'message' => "Transactions successfully added", 'app' => $key);
                    if($this->input->post('bankTransId')){
                        $this->db->update('plaid_trans', array('trans_match' => $rec_id), array('transaction_id' => $this->input->post('bankTransId')));
                        $response1['recId'] = $rec_id;
                        $response1['transId'] = $this->input->post('bankTransId');
                ...
                $this->validate_model->validation['bool'] = true;
                $this->validate_model->validation['msg'] = '';
                echo json_encode(array('type' => $validate['type'],'auth' => $validate['auth'], 'message' => "Transaction not added <br> $errors", 'app' => $key));
            }
    }
    ...
    //paying bills for all vendors at once
    public function payBillsMultgetModal()
    {
        $this->load->model('payBills_model');
    ...
    public function payBillsgetModal()
    {
        ...
        switch ($this->input->post('mode')) {
            case 'add' :
                $this->data['target'] = 'transactions/addBillPayment';
                $this->data['title'] = 'Pay Bills';
                $this->data['edit'] = 'edit';
                $this->data['vendors'] = $this->payBills_model->getVendor();
                $this->data['properties'] = $this->payBills_model->getProperties();
                //$this->data['header'] = $this->payBills_model->getHeaderEdit($this->input->post('id'));
                $this->data['header']->profile_id = $vendorId;
            ...
            case 'edit' ://which values are we passing into the edit 
                ...
                $clrd = $recInfo->clr == 1 ? 'clrdrecinfo' : 'unclrdrecinfo';
                ...
                $this->data['RecIdHtml'] = $recInfo->rec_id != null && $recInfo->clr == 1 ? '<span data-id="50" class="'.$clrd.' reportLink" rtype="report" defaults="'.$recInfo->rec_id.'">Cleared on Rec# '.$recInfo->rec_id.'</span>' : '';
    ...
    function applyReceivedPayments()
    {
        ...
        if ($validate['bool'] && $validate['auth'] =='pass' && /*$this->form_validation->run() &&*/
            ...
            $array = ['type' => $validate['type'],'auth' => $validate['auth'], 'message' => "Payment successfully applied."];
                if($this->input->post('bankTransId')){
                $this->db->update('plaid_trans', array('trans_match' => $rec_id), array('transaction_id' => $this->input->post('bankTransId')));
                $array['recId'] = $rec_id;
                $array['transId'] = $this->input->post('bankTransId');
            ...
            $errors = $validate['msg'];
            //$errors = $errors . validation_errors() ."</br>". $validate['msg'];
            echo json_encode(array('type' => 'danger', 'message' => $errors,'auth' => $validate['auth']));
    ...
    public function receivePaymentsgetModal()// not complete see my tasks in asana
    {
        ...
        switch ($this->input->post('mode')) {
            ...
            case 'edit' ://which values are we passing into the edit 
                ...
                $header = $this->receivePayments_model->getHeaderEdit($this->input->post('id'));
                ...
                $clrd = $this->data['header']->clr == 1 ? 'clrdrecinfo' : 'unclrdrecinfo';
                $this->data['hasRecId'] = $this->data['header']->rec_id != null && $this->data['header']->clr == 1 ? 'hasRecId' : '';
                $this->data['hasRecIdHtml'] = $this->data['header']->rec_id != null && $this->data['header']->clr == 1 ? 'Cleared' : '';
                $this->data['RecIdHtml'] = $this->data['header']->rec_id != null  ? ' <span data-id="50" class ="'.$clrd.' reportLink" rtype="report" defaults="'.$this->data['header']->rec_id.'"> Cleared on Rec#'.$this->data['header']->rec_id.'</span>'  : '';
                // if (isset($params->es_key)) {
                //     $key = explode('.', $params->es_key)[1];
                //     $this->data['account'] = new stdClass();
                //     $this->data['account']->$key = $params->es_value;
...
/////utilities
    ...
    public function quickAdd(){
        $data = $this->input->post();
        $_POST['bankTransId'] = $data['bank_trans_id'];
        //$bankTransId = $this->input->post('bank_trans_id');
        
        $_POST['radioButton'] = 'normal'; // or credit
        if($data['amount'] < 0){
            $_POST['radioButton'] = 'credit';
            $data['amount'] = 0 - $data['amount'];
        }
        $_POST['headerTransaction']['account_id'] = $data['bank_account']; // need to get bank account id
        //$_POST['headerTransaction']['rec_id'] = $this->input->post('bank_trans_id');
        $_POST['headerTransaction']['property_id']= $data['property_id']; 
        $_POST['headerTransaction']['profile_id']= $data['profile_id'];
        $_POST['header']['transaction_ref']= $data['bank_trans_id'];
        $_POST['header']['transaction_date']= $data['date'];
        $_POST['headerTransaction']['credit'] = $data['amount'];
        $_POST['header']['memo']= $data['description'];
        $_POST['transactions'][0]['account_id']= $data['account_id'];
        $_POST['transactions'][0]['property_id']= $data['property_id'];
        $_POST['transactions'][0]['unit_id']= '';
        $_POST['transactions'][0]['description']= $data['description'];
        $_POST['transactions'][0]['debit'] = $data['amount'];
        $_POST['transactions'][0]['class_id']= '';
        //$_POST['transactions'][0]['profile_id']= 18784;
        $result = $this->addBankTrans();
        echo $result;
    }
    ...
    public function sendEmail($row2, $type)
    {
        $this->load->library('email');
        ...
        $row = $type == 'c'?json_decode(json_encode($row2), FALSE):$row2->data;
        $trans_date = $type == 'c'? $row->date : $row->transaction_date;
        ...
        $lease=$lquery->row();
        if(($tenant->email_pay_notifications != 1 && $type == 'p') or ($tenant->email_charge_notifications != 1 && $type == 'c')) {return;}
        ...
        //echo $tenant->email;
            ...
            $subject = $type == 'c'? 'Rent Reminder From '.$company_name: $company_name. ' - Payment Confirmation' ;
            $message = $type == 'c'? 'This is a reminder that your rent payment of $'.number_format($row->credit, 2, '.', ',').' is due on '.$trans_date.'.': 'Thank you for your payment of $'.number_format($row->credit, 2, '.', ',').' on '.$trans_date.'.';
                            ...
                            <td align="center" valign="top" style="background: #ffffff;">
                                 ...
                                 <tr>
                                    <td class="mob_left" align="left" valign="top">
                                       <font class="mob_title1" face="\'Source Sans Pro\', sans-serif" color="#1a1a1a" style="font-size: 52px; line-height: 55px; font-weight: 300; ">
                                          <span class="mob_title1" style="font-family: \'Source Sans Pro\', Arial, Tahoma, Geneva, sans-serif; color: #1a1a1a; font-size: 16px; line-height: 32px; font-weight: 300;">Hello '.$tenant->first_name.' '.$tenant->last_name.',
                                          <br>
                              ...
                              <tbody><tr>
                                 ...
                                 <a href="https://simpli-city.com/"style="text-decoration: none;">
                                   ...
                                   <img style="width: 70px;" src="'.base_url().'uploads/images/logo-dark.png"></a>
