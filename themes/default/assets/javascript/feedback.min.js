...
(function( window, document, undefined ) {
    ...
    window.Feedback = function( options ) {
        ...
        // support emails
        options.supportEmails = options.supportEmails || [{name: 'Wade Wilson', userId: 57, email: 'admin@admin.com'}];
            ...
            send: function( adapter ) {
                ...
                adapter.send(data, function( success ) {
                    ...
                    modalBody.appendChild(
                      success ? 
                      document.createTextNode(options.messageSuccess) : 
                      document.createTextNode(options.messageError)
                    );                  
                } );
        ...
        button.className = "feedback-btn feedback-bottom-right no-print";
        ...
        window.Feedback.options = options;
        
        return returnMethods;
    };
  
    window.Feedback.selectedSupportEmail = 57;
    ...
    window.Feedback.Form.prototype.review = function(dom, options) {
      dom.appendChild(element("b", "Send to"));
      dom.appendChild(element("br", ""));
  
      // Add wrapper for selector and ▼ icon
      const selectWrap = document.createElement("div");
      $(selectWrap).addClass("feedback-support-selector-wrap");
      dom.appendChild(selectWrap);
  
  
      const selectList = document.createElement("select");
      $(selectList).addClass("feedback-support-selector");
      $(selectList).change(function() {
          window.Feedback.selectedSupportEmail = $(this).val()
      });
      selectWrap.appendChild(selectList);
      
      const selectIcon = document.createElement("div");
      $(selectIcon).addClass("toggle collapse");
      selectWrap.appendChild(selectIcon);
  
      options.supportEmails.forEach((item) => {
          const option = document.createElement("option");
          option.value = item.userId;
          option.text = `${item.name} (${item.email})`;
      ...
      this.elements.forEach((item) => {
          if (item.element.value.length > 0) {
              dom.appendChild(element("b", item.name));
              dom.appendChild(element("br"));
    ...
    window.Feedback.Review.prototype.render = function(pages) {
        ...
        for (let i=0; i<len; i++) {
            ...
            pages[i].review(this.dom, window.Feedback.options);
        }
    ...
    window.Feedback.Review.prototype.data = function() {
      return {"userId": window.Feedback.selectedSupportEmail};
    };
            ...
            try {
                ...
                options.onrendered = options.onrendered || function( canvas ) {
                  //   $this.h2cCanvas = canvas;
                    $this.h2cCanvas = document.createElement( "canvas" );
                    thisCtx = $this.h2cCanvas.getContext('2d');
  
                    $this.h2cCanvas.width = canvas.width / window.devicePixelRatio;
                    $this.h2cCanvas.height = canvas.height / window.devicePixelRatio;
  
                    thisCtx.drawImage(canvas, 0, 0 , $this.h2cCanvas.width, $this.h2cCanvas.height);
  
  
                    $this.h2cDone = true;
                };
    
                window.html2canvas(document.body).then(options.onrendered);
