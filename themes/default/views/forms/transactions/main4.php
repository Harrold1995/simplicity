				...
				<table class="table-c mobile-hide dataTable no-footer treetable tree-table" style="display: table; width: 100%;" id="transactions-table">
					<thead class="dataTables_scrollHead" style="display: table; width: 100%; table-layout: fixed; overflow: hidden;">
						<tr>
							...
							<th id="sort_by_balance"  width="10%" class="text-center" style="border: 1px solid black;">balance</th>
							<th id="show_all" width="10%" style="border: 1px solid black;">Show all</th>
...
var eventlistenerArray = [];
   ...
   function showAll(){
    $( "#transactionstable" ).find( "tr" ).show();
  }
...
//    console.log(eventlistenerArray[i]);
   $('body').off( "click", eventlistenerArray[i] );}
    ...
    $('body').on('click', '#show_all', function () {removeAllListeners(); showAll();});
...
function addRowToTransactionsForm(body,sortBy) {
    ...
    var colors = ["green","yellow","aqua","chartreuse", "pink", "orange"];
    var a;
    var amount = 10;
    var row = 1;   
    function recursion(data, parent){
        ...
        for (var i = 0; i < data.length; i++) {
            if(typeof data[i].key !== 'undefined'){
                    ...
                    zz = zz.replace(/ /g,'');
                    zz = zz.replace(/&/g,"");
                     testToggle(parent + '-' + zz, row);
            newRow += '<tr class="'+ parent + ' '+ row +'" id="'+ parent + '-'+ zz + '" style="display:none; margin-left:' + amount + 'px;"><td style="background-color:' + colors[a] + '">Name: ' + data[i].key; + '</td></tr>';
              if(typeof data[i].values !== 'undefined'){
                  a++;
                  amount = amount + 10;
                  //row++;
                  recursion(data[i].values, parent + '-'+ zz);
                  a--;
                  amount = amount - 10;
                }else{
                  a++;
                  amount = amount + 10;
                  newRow += '<tr class="'+ parent + '-'+ zz + ' '+ row +'" id="'+ parent + '-'+ zz + '" style="display:none; margin-left:' + amount + 'px;"><td style="background-color:' + colors[a] + '">Name: ' + data[i].value; + '</td></tr>';
                  a--;
                  amount = amount - 10;
                }
            }else{
                    a++;
                    amount = amount + 10;
                    newRow += '<tr class="'+ parent + '" data-id="" style="display: none; margin-left:40px;">';
                       $.each(data[i], function(key, value) {
                       newRow += '<td style="background-color: ' + colors[a] + ';">'+ key + '</br> ' + value +'</td>';
                 });
                 newRow += '</tr>';
                 a--;
                 amount = amount - 10;
...
for (var i = 0; i < expensesByName.length; i++) {
    var z = expensesByName[i].key;
         ...
         testToggle(z);
         row++;
    newRow += '<tr class="d3_table_account_categories '+row + '" id="' + z + '"><td>Name: ' + expensesByName[i].key + '</td></tr>';
    var a =1;
    recursion(expensesByName[i].values, z);
}
